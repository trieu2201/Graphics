...
def main():
    ...
    viewer = Viewer()
    function = lambda x, y: math.sin(x) / (x * x + 1) + math.sin(y) / (y * y + 1)
    # place instances of our basic objects

    model_gouraud = Mesh("./gouraud.vert", "./gouraud.frag", function).setup()
    gouraudPos = glm.translate(glm.mat4(1.0), glm.vec3(0.0, 0.0, 0.0))

    model_phong = Mesh("./phong.vert", "./phong.frag", function).setup()
    phongPos = glm.translate(glm.mat4(1.0), glm.vec3(-4.0, 0.0, 0.0))

    model_textured = Mesh("./phong.vert", "./phong.frag", function).setup(1)
